...
class Classifier(nn.Module):
    ...
    '''
    def __init__(self, embed_path, embed_size, n_classes, device, hidden_size=128, num_layers=1):
        '''
        Initializes class variables.

        input : embed_path <str> : path where embeddings are stored 
        ...
        self.embeds, self.id_map = self.load_embeds(embed_path)
        ...
        self.out1 = nn.Linear(2*hidden_size, n_classes)
        self.out2 = nn.Linear(2*hidden_size, n_classes)
        self.out3 = nn.Linear(2*hidden_size, n_classes)
        self.out4 = nn.Linear(2*hidden_size, n_classes)
        self.out5 = nn.Linear(2*hidden_size, n_classes)
        self.out6 = nn.Linear(2*hidden_size, n_classes)
        self.out7 = nn.Linear(2*hidden_size, n_classes)
        self.out8 = nn.Linear(2*hidden_size, n_classes)
        self.out9 = nn.Linear(2*hidden_size, n_classes)
        self.out10 = nn.Linear(2*hidden_size, n_classes)
        self.out11 = nn.Linear(2*hidden_size, n_classes)
        self.out12 = nn.Linear(2*hidden_size, n_classes)
        self.out13 = nn.Linear(2*hidden_size, n_classes)
        self.out14 = nn.Linear(2*hidden_size, n_classes)
        self.out15 = nn.Linear(2*hidden_size, n_classes)
        self.out16 = nn.Linear(2*hidden_size, n_classes)
        self.out17 = nn.Linear(2*hidden_size, n_classes)
        self.out18 = nn.Linear(2*hidden_size, n_classes)
        self.out19 = nn.Linear(2*hidden_size, n_classes)
        self.out20 = nn.Linear(2*hidden_size, n_classes)
        self.out21 = nn.Linear(2*hidden_size, n_classes)
        self.out22 = nn.Linear(2*hidden_size, n_classes)
        self.out23 = nn.Linear(2*hidden_size, n_classes)
        self.out24 = nn.Linear(2*hidden_size, n_classes)
        self.out25 = nn.Linear(2*hidden_size, n_classes)
        self.out26 = nn.Linear(2*hidden_size, n_classes)
        self.out27 = nn.Linear(2*hidden_size, n_classes)
        self.out28 = nn.Linear(2*hidden_size, n_classes)
        
        # Define Softmax Layer for each class
        self.softmax1 = nn.LogSoftmax(dim=1)
        self.softmax2 = nn.LogSoftmax(dim=1)
        self.softmax3 = nn.LogSoftmax(dim=1)
        self.softmax4 = nn.LogSoftmax(dim=1)
        self.softmax5 = nn.LogSoftmax(dim=1)
        self.softmax6 = nn.LogSoftmax(dim=1)
        self.softmax7 = nn.LogSoftmax(dim=1)
        self.softmax8 = nn.LogSoftmax(dim=1)
        self.softmax9 = nn.LogSoftmax(dim=1)
        self.softmax10 = nn.LogSoftmax(dim=1)
        self.softmax11 = nn.LogSoftmax(dim=1)
        self.softmax12 = nn.LogSoftmax(dim=1)
        self.softmax13 = nn.LogSoftmax(dim=1)
        self.softmax14 = nn.LogSoftmax(dim=1)
        self.softmax15 = nn.LogSoftmax(dim=1)
        self.softmax16 = nn.LogSoftmax(dim=1)
        self.softmax17 = nn.LogSoftmax(dim=1)
        self.softmax18 = nn.LogSoftmax(dim=1)
        self.softmax19 = nn.LogSoftmax(dim=1)
        self.softmax20 = nn.LogSoftmax(dim=1)
        self.softmax21 = nn.LogSoftmax(dim=1)
        self.softmax22 = nn.LogSoftmax(dim=1)
        self.softmax23 = nn.LogSoftmax(dim=1)
        self.softmax24 = nn.LogSoftmax(dim=1)
        self.softmax25 = nn.LogSoftmax(dim=1)
        self.softmax26 = nn.LogSoftmax(dim=1)
        self.softmax27 = nn.LogSoftmax(dim=1)
        self.softmax28 = nn.LogSoftmax(dim=1)

    def load_embeds(self, data_dir):
        embeds = np.load(data_dir + "/val.npy")
        id_map = {}
        with open(data_dir + "/val.txt") as fp:
            for i, line in enumerate(fp):
                id_map[int(line.strip())] = i
        return embeds, id_map

    def get_embeds(self, ids):
        batch_embeds = self.embeds[[self.id_map[id.item()] for id in ids]] 
        return torch.FloatTensor(batch_embeds).to(device=self.device)
    ...
    def forward(self, inp, batch_size):
        ...
        output_cell_junc = self.softmax1(self.out1(output_cell_junc)) #[batch_size, n_classes]
        ...
        output_fas = self.softmax2(self.out2(output_fas)) #[batch_size, n_classes]
        ...
        output_cyto_bridge = self.softmax3(self.out3(output_cyto_bridge)) #[batch_size, n_classes]
        ...
        output_midbody = self.softmax4(self.out4(output_midbody)) #[batch_size, n_classes]
        ...
        output_midr = self.softmax5(self.out5(output_midr))   #[batch_size, n_classes]
        ...
        output_plasma_memb = self.softmax6(self.out6(output_plasma_memb)) #[batch_size, n_classes]
        ...
        output_cytosol = self.softmax7(self.out7(output_cytosol)) #[batch_size, n_classes]
        ...
        output_vesicles = self.softmax8(self.out8(output_vesicles)) #[batch_size, n_classes]
        ...
        output_er = self.softmax9(self.out9(output_er))   #[batch_size, n_classes]
        ...
        output_golgi = self.softmax10(self.out10(output_golgi))   #[batch_size, n_classes]
        ...
        output_endo = self.softmax11(self.out11(output_endo))   #[batch_size, n_classes]
        ...
        output_mito = self.softmax12(self.out12(output_mito))   #[batch_size, n_classes]
        ...
        output_pero = self.softmax13(self.out13(output_pero))   #[batch_size, n_classes]
        ...
        output_lyso = self.softmax14(self.out14(output_lyso))   #[batch_size, n_classes]
        ...
        output_nucl_memb = self.softmax15(self.out15(output_nucl_memb))   #[batch_size, n_classes]
        ...
        output_nucleoplasm = self.softmax16(self.out16(output_nucleoplasm)) #[batch_size, n_classes]
        ...
        output_nucl_bod = self.softmax17(self.out17(output_nucl_bod))   #[batch_size, n_classes]
        ...
        output_nucl_spec = self.softmax18(self.out18(output_nucl_spec))   #[batch_size, n_classes]
        ...
        output_nucleoli = self.softmax19(self.out19(output_nucleoli)) #[batch_size, n_classes]
        ...
        output_nfc = self.softmax20(self.out20(output_nfc))   #[batch_size, n_classes]
        ...
        output_cyto_bodies = self.softmax21(self.out21(output_cyto_bodies)) #[batch_size, n_classes]
        ...
        output_lipid = self.softmax22(self.out22(output_lipid)) #[batch_size, n_classes]
        ...
        output_inter_fila = self.softmax23(self.out23(output_inter_fila)) #[batch_size, n_classes]
        ...
        output_actin_fila = self.softmax24(self.out24(output_actin_fila)) #[batch_size, n_classes]
        ...
        output_micro = self.softmax25(self.out25(output_micro)) #[batch_size, n_classes]
        ...
        output_mito_spindle = self.softmax26(self.out26(output_mito_spindle))   #[batch_size, n_classes]
        ...
        output_centrosome = self.softmax27(self.out27(output_centrosome))   #[batch_size, n_classes]
        ...
        output_centr_sat = self.softmax28(self.out28(output_centr_sat))   #[batch_size, n_classes]
